...
angular.module('myApp.controllers', ['nvd3ChartDirectives'])
  .controller('HomeCtrl', ['$scope', '$http', '$location', 'userService', function($scope, $http, $location, User) {
      ...
      $scope.reportProblem = function() {
        ...
        $http({method: 'POST', url: '../php/problem.php?difficulty='+$scope.prevDiff+'&success='+correct+'&argOne='+$scope.numone+'&argTwo='+$scope.numtwo+'&operation='+op+'&correct='+$scope.check()+'&answer='+$scope.answer+'&userId='+User.userId}).
      ...
      $scope.newProblem = function() {
        ...
        $scope.alert = "warning";
        $scope.messagePre = "Didn't like that one? ";
        $scope.message = "Hopefully this problem is more to your liking.";
        if($scope.addition){
          ops += "+";
        }
        if($scope.subtraction){
